...
module.exports = {
    Name: 'taglılarım',
    Aliases: ['taglılar'],
    Description: 'Taglı verilerinizi gösterir.',
    Usage: 'taglılarım <@User/ID>',
    Category: 'Auth',
    Cooldown: 0,
    Command: { Prefix: true },
    ...
    messageRun: async (client, message, args) => {
        ...
        let page = 1
        const totalPages = Math.ceil(document.Taggeds.length / 10)
        const mappedPages = document.Taggeds.map((t, i) => `${bold((i + 1).toString())}. ${time(Math.floor(t.timestamp / 1000), 'f')}: ${userMention(t.id)} (${inlineCode(t.id)})`)

        const embed = new EmbedBuilder({
            color: client.random(),
            description: [
                `${member} adlı yetkilinin taglı seçtiği kullanıcı bilgileri;\n`,
                mappedPages.slice(0, 10).join('\n')
            ].join('\n'),
            footer: { text: `Sunucumuza ${document.Taggeds.length} taglı kazandırmış. Tebrikler!` }
        })

        const question = await message.channel.send({
            embeds: [embed],
            components: document.Taggeds.length > 5 ? [client.getButton(page, totalPages)] : []
        })

        if (10 >= document.Taggeds.length) return
