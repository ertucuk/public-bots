...
module.exports = {
    ...
    messageRun: async (client, message, args) => {
        ...
        if (message.member.permissions.has(Flags.MoveMembers) || message.guild.settings.transporterRoles.some((e) => message.member.roles.cache.has(e))) {
            ...
            message.reply({ embeds: [new global.VanteEmbed().setDescription(`${client.getEmoji("check")} Başarıyla ${member} adlı üyeyi \`${message.member.voice.channel.name}\` adlı kanala taşıdınız.`)] });
        } else {
            ...
            const msg = await message.reply({
                ...
                embeds: [new global.VanteEmbed().setDescription(`${message.author} adlı üye seni \`${message.member.voice.channel.name}\` kanalına çekmek istiyor. Onaylıyor musun? İşlem 30 saniye sonra iptal edilecektir.`)],
                components: [row],
            });
            ...
            collector.on('collect', async (i) => {
                if (i.customId === 'accept') {
                    ...
                    msg.edit({ embeds: [new global.VanteEmbed().setDescription(`${client.getEmoji("check")} Başarıyla ${member} adlı üyeyi \`${message.member.voice.channel.name}\` adlı kanala taşıdınız.`)], components: [] });
